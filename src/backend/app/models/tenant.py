...
class Tenant(BaseModel, SoftDeleteMixin):
    ...
    features_enabled: Mapped[list[str] | None] = mapped_column(
        ARRAY(Text),
        default=list,
        nullable=True,
    )
...
class Workspace(BaseModel, SoftDeleteMixin):
    ...
    settings: Mapped[dict[str, Any] | None] = mapped_column(JSONB, default=dict, nullable=True)
    branding: Mapped[dict[str, Any] | None] = mapped_column(JSONB, default=dict, nullable=True)
    ...
    __table_args__ = (Index("ix_workspace_tenant_slug", "tenant_id", "slug", unique=True),)
...
class TeamMember(BaseModel):
    ...
    __table_args__ = (Index("ix_team_member_team_user", "team_id", "user_id", unique=True),)
...
class RolePermission(BaseModel):
    ...
    __table_args__ = (Index("ix_role_permission_unique", "role_id", "permission_id", unique=True),)
