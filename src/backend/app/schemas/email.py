...
class SendRequest(BaseModel):
    ...
    body_text: str | None = Field(
        None, description="Plain-text body (required if body_html is not set)"
    )
    template_id: UUID | None = Field(None, description="Use an existing email template")
    template_variables: dict[str, Any] | None = Field(
        None,
        description="Variable substitutions for template rendering",
    )
    ...
    def _validate_body(cls, v: str | None, info: Any) -> str | None:
        ...
        values = info.data
        if (
            not values.get("template_id")
            and not v
            and not values.get("body_text")
            and not values.get("body_html")
        ):
...
class SendBulkItem(BaseModel):
    ...
    variables: dict[str, Any] | None = Field(
        None,
        description="Per-recipient template variable overrides",
    )
...
class EmailTemplateCreate(BaseModel):
    ...
    variables: list[str] | None = Field(
        None,
        description="List of expected template variable names",
    )
...
class BounceWebhookPayload(BaseModel):
    ...
    event_type: str | None = Field(
        None,
        pattern=r"^(bounce|complaint|delivery|open|click)$",
    )
    recipient_email: str | None = None
    bounce_type: str | None = Field(
        None,
        pattern=r"^(permanent|transient|undetermined)$",
    )
